...
<div>
  ...
  <ol class="list-inside list-decimal text-sm">
    {#each data.required_skills.get('Fundamentals') as skill, index}
      <li value={index + 1}>{skill.title}</li>
    {/each}
  </ol>
</div>
...
<div class="flex flex-col gap-3">
  ...
  {#each data.users as user (user.user_id)}
    <div class="border-1 border-(--light-color) bg-white dark:bg-(--dark-color) rounded-md shadow-md p-3">
      <div class="font-semibold mb-2">
        {user.name}
      </div>
      <div class="flex flex-wrap gap-1.5">
        {#each data.required_skills.get('Fundamentals') as skill, index (skill.slug)}
